(function($){
		...
		wp.media.customlibEditGallery1 = {
			frame: function () {
				...
				this._frame.on('update', function () {
					...
					var data = {
						action: 'biagiotti_mikado_gallery_upload_get_images',
						ids: ids,
						post_name: $input_gallery_items.attr('name'),
						gallery_upload_get_images: $('#mkdf_gallery_upload_get_images_' + $input_gallery_items.attr('name')).val()
					};
	...
	function mkdfInitAjaxForm() {
		...
		$(document).delegate(".mkdf_ajax_form", "submit", function (a) {
			var b = $(this),
				c = {
					action: "biagiotti_mikado_save_options"
				};
		...
		if (holder.length) {
			...
			mkdfImportBtn.on('click', function (e) {
				...
				if (confirm('Are you sure, you want to import Options now?')) {
					...
					var data = {
						action: 'biagiotti_core_import_theme_options',
						content: importValue,
						nonce: importNonce
					};
		...
		if (holder.length) {
			...
			mkdfImportBtn.on('click', function (e) {
				...
				if (confirm('Are you sure, you want to import Custom Sidebars now?')) {
					...
					var data = {
						action: 'biagiotti_core_import_custom_sidebars',
						content: importValue,
						nonce: importNonce
					};
		...
		if (holder.length) {
			...
			mkdfImportBtn.on('click', function (e) {
				...
				if (confirm('Are you sure, you want to import Widgets now?')) {
					...
					var data = {
						action: 'biagiotti_core_import_widgets',
						content: importValue,
						nonce: importNonce
					};
