(function($) {
	...
	$(document).ready(function(){
		mkdfInitWidgetFieldColorPicker();
	});
	...
	$( document ).on( 'widget-added widget-updated', function(event, widget){
		mkdfColorPickeronFormUpdate(event, widget);
		mkdfButtonWidgetFieldDependency();
		mkdfIconWidgetFieldDependency();
		mkdfImageGalleryWidgetFieldDependency();
		mkdfSocialIconWidgetFieldDependency();
	...
	function mkdfInitColorPicker(widget) {
		if (widget.find('.wp-picker-container').length <= 0) {
			widget.find('input.mkdf-color-picker-field').wpColorPicker({
	...
	function mkdfColorPickeronFormUpdate( event, widget ) {
		mkdfInitColorPicker( widget );
	}
	
	function mkdfInitWidgetFieldColorPicker() {
		var colorPicker = $('.mkdf-widget-color-picker');
		...
		if (colorPicker.length && colorPicker.find('.wp-picker-container').length <= 0) {
			colorPicker.each(function(){
				...
				if (listParent.length <= 0){
					mkdfInitColorPicker( thisColorPicker );
				}
			});
		}
	}
	...
	function mkdfSocialIconWidgetFieldDependency() {
		...
		var IconText = {
			...
			'square-icon': ['icon_pack','fa_icon','fe_icon','ion_icon','simple_line_icon','skin' ],
			'text': 'social_text'
		};
		...
		mkdfUpdateWidgetSelection('mkdf_social_icon_widget', 'icon_text', IconText);
		mkdfUpdateWidgetSelection('mkdf_social_icons_group_widget', 'layout', Layout);
		...
		mkdfUpdateWidgetSelection('mkdf_social_icons_group_widget', 'icon_pack', iconPacks);
	}
