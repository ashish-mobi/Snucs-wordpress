...
class BiagiottiMikadoClassLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_biagiotti_mikado_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_biagiotti_mikado_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'biagiotti' );
		
		if ( isset( $_COOKIE[ 'mkdf-like_' . $post->ID ] ) ) {
			$class = 'mkdf-like liked';
			$title = esc_html__( 'You already like this!', 'biagiotti' );
		}
...
if ( ! function_exists( 'biagiotti_mikado_create_like' ) ) {
	function biagiotti_mikado_create_like() {
		BiagiottiMikadoClassLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'biagiotti_mikado_create_like' );
}