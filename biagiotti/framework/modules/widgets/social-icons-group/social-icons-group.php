...
if ( class_exists( 'BiagiottiCoreClassWidget' ) ) {
	class BiagiottiMikadoClassClassIconsGroupWidget extends BiagiottiCoreClassWidget {
		public function __construct() {
			parent::__construct(
				'mkdf_social_icons_group_widget',
				esc_html__( 'Biagiotti Social Icons Group Widget', 'biagiotti' ),
				array( 'description' => esc_html__( 'Use this widget to add a group of up to 6 social icons to a widget area.', 'biagiotti' ) )
			);
		...
		protected function setParams() {
			$this->params = array_merge(
				...
				biagiotti_mikado_icon_collections()->getSocialIconWidgetMultipleParamsArray( 6 ),
				array(
					array(
						'type'    => 'dropdown',
						'name'    => 'layout',
						'title'   => esc_html__( 'Icons Layout', 'biagiotti' ),
						'options' => array(
							''             => esc_html__( 'Default', 'biagiotti' ),
							'square-icons' => esc_html__( 'Square', 'biagiotti' ),
						)
					),
					array(
						'type'        => 'dropdown',
					...
					array(
						'type'    => 'dropdown',
						'name'    => 'alignment',
						'title'   => esc_html__( 'Text Alignment', 'biagiotti' ),
						'options' => array(
					...
					array(
						'type'  => 'textfield',
						'name'  => 'icon_size',
						'title' => esc_html__( 'Icons Size (px)', 'biagiotti' )
					),
					array(
						'type'  => 'colorpicker',
						'name'  => 'color',
		...
		public function widget( $args, $instance ) {
			...
			for ( $n = 1; $n <= 6; $n ++ ) {
				...
				<?php if ( ! empty( $icon_holder_html ) ) { ?>
					<a class="mkdf-social-icon-widget-holder mkdf-icon-has-hover" <?php echo biagiotti_mikado_get_inline_attr( $hover_color, 'data-hover-color' ); ?> <?php biagiotti_mikado_inline_style( $icon_styles ) ?> href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr( $target ); ?>">
