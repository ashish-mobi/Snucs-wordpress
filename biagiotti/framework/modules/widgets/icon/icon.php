...
if ( class_exists( 'BiagiottiCoreClassWidget' ) ) {
	class BiagiottiMikadoClassIconWidget extends BiagiottiCoreClassWidget {
		public function __construct() {
			parent::__construct(
				'mkdf_icon_widget',
				esc_html__( 'Biagiotti Icon Widget', 'biagiotti' ),
				array( 'description' => esc_html__( 'Add icons to widget areas', 'biagiotti' ) )
			);
		...
		protected function setParams() {
			$this->params = array_merge(
				biagiotti_mikado_icon_collections()->getIconWidgetParamsArray(),
				array(
					array(
						'type'  => 'textfield',
						'name'  => 'icon_text',
						'title' => esc_html__( 'Icon Text', 'biagiotti' )
					),
					array(
						'type'  => 'textfield',
						'name'  => 'link',
					...
					array(
						'type'  => 'textfield',
						'name'  => 'text_size',
						'title' => esc_html__( 'Text Size (px)', 'biagiotti' )
					),
					array(
						'type'    => 'dropdown',
						'name'    => 'text_style',
					...
					array(
						'type'  => 'colorpicker',
						'name'  => 'text_color',
						'title' => esc_html__( 'Text Color', 'biagiotti' )
					),
					array(
						'type'        => 'textfield',
						'name'        => 'margin',
		...
		public function widget( $args, $instance ) {
			...
			$data   = array();
			$data[] = ! empty( $instance['icon_hover_color'] ) ? biagiotti_mikado_get_inline_attr( $instance['icon_hover_color'], 'data-hover-color' ) : '';
			$data   = implode( ' ', $data );
			...
			if ( ! empty( $instance['text_size'] ) ) {
				$text_styles[] = 'font-size: ' . biagiotti_mikado_filter_px( $instance['text_size'] ) . 'px';
			}
			if ( ! empty( $instance['text_style'] ) ) {
				$text_styles[] = 'font-style: ' . $instance['text_style'];
			}
			...
			if ( ! empty( $instance['icon_pack'] ) ) {
				...
				if ( ! empty( $icon_class ) ) {
					...
					$icon_holder_html = '<span class="mkdf-icon-element ' . esc_attr( $icon_class ) . '" ' . biagiotti_mikado_get_inline_style( $icon_styles ) . '></span>';
				}
			}
			...
			if ( ! empty( $instance['icon_text'] ) ) {
				$icon_text_html = '<span class="mkdf-icon-text ' . esc_attr( $icon_text_class ) . '" ' . biagiotti_mikado_get_inline_style( $text_styles ) . '>' . esc_html( $instance['icon_text'] ) . '</span>';
			}
			?>
			
			<a <?php biagiotti_mikado_class_attribute( $holder_classes ); ?> <?php echo wp_kses_post( $data ); ?> href="<?php echo esc_url( $link ); ?>" target="<?php echo esc_attr( $target ); ?>" <?php echo biagiotti_mikado_get_inline_style( $holder_styles ); ?>>
