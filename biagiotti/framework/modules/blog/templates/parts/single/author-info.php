<?php
$author_info_box       = esc_attr( biagiotti_mikado_options()->getOptionValue( 'blog_author_info' ) );
$author_info_email     = esc_attr( biagiotti_mikado_options()->getOptionValue( 'blog_author_info_email' ) );
$author_id             = esc_attr( get_the_author_meta( 'ID' ) );
$social_networks       = biagiotti_mikado_is_plugin_installed( 'core' ) ? biagiotti_mikado_get_user_custom_fields() : false;
$display_author_social = biagiotti_mikado_options()->getOptionValue( 'blog_single_author_social' ) === 'no' ? false : true;
?>
	...
	<div class="mkdf-author-description">
		<div class="mkdf-author-description-image">
			<a itemprop="url" href="<?php echo esc_url( get_author_posts_url( $author_id ) ); ?>" title="<?php the_title_attribute(); ?>">
				<?php echo biagiotti_mikado_kses_img( get_avatar( get_the_author_meta( 'ID' ), 145 ) ); ?>
			</a>
		</div>
					...
					<div class="mkdf-author-social-icons clearfix">
						<?php foreach ( $social_networks as $network ) { ?>
							<a itemprop="url" href="<?php echo esc_attr( $network['link'] ) ?>" target="_blank">
								<?php echo biagiotti_mikado_icon_collections()->renderIcon( $network['class'], 'ion_icons' ); ?>
							</a>
						<?php } ?>
					</div>
				<?php } ?>
