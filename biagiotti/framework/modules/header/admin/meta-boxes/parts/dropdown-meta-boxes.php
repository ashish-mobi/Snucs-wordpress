...
if ( ! function_exists( 'biagiotti_mikado_get_hide_dep_for_dropdown_meta_boxes' ) ) {
	function biagiotti_mikado_get_hide_dep_for_dropdown_meta_boxes() {
		$hide_dep_options = apply_filters( 'biagiotti_mikado_filter_dropdown_hide_meta_boxes', $hide_dep_options = array() );
		
		return $hide_dep_options;
	}
}

if ( ! function_exists( 'biagiotti_mikado_dropdown_meta_options_map' ) ) {
	function biagiotti_mikado_dropdown_meta_options_map( $header_meta_box ) {
		$hide_dep_widgets 			= biagiotti_mikado_get_hide_dep_for_dropdown_meta_boxes();

		$dropdown_container = biagiotti_mikado_add_admin_container_no_style(
			array(
				'type'       => 'container',
		...
		biagiotti_mikado_add_admin_section_title(
			array(
				...
				'title'  => esc_html__( 'Dropdown Styles', 'biagiotti' )
			)
		);
		...
		do_action( 'biagiotti_mikado_dropdown_additional_meta_boxes_map', $dropdown_container );
	}
	
	add_action( 'biagiotti_mikado_action_dropdown_meta_boxes_map', 'biagiotti_mikado_dropdown_meta_options_map', 10, 1 );
}