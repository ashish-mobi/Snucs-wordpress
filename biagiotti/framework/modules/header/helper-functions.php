...
if ( ! function_exists( 'biagiotti_mikado_header_skin_class' ) ) {
	/**
	 * Function that adds header style class to body tag
	 */
	function biagiotti_mikado_header_skin_class( $classes ) {
		$header_style     = biagiotti_mikado_get_meta_field_intersect( 'header_style', biagiotti_mikado_get_page_id() );
		$header_style_404 = biagiotti_mikado_options()->getOptionValue( '404_header_style' );
	...
	add_filter( 'body_class', 'biagiotti_mikado_header_skin_class' );
}

if ( ! function_exists( 'biagiotti_mikado_sticky_header_behaviour_class' ) ) {
	/**
	 * Function that adds header behavior class to body tag
	 */
	function biagiotti_mikado_sticky_header_behaviour_class( $classes ) {
		$header_behavior = biagiotti_mikado_get_meta_field_intersect( 'header_behaviour', biagiotti_mikado_get_page_id() );
	...
	add_filter( 'body_class', 'biagiotti_mikado_sticky_header_behaviour_class' );
}

if ( ! function_exists( 'biagiotti_mikado_menu_dropdown_appearance' ) ) {
	/**
	 ...
	 */
	function biagiotti_mikado_menu_dropdown_appearance( $classes ) {
		$dropdown_menu_appearance = biagiotti_mikado_options()->getOptionValue( 'menu_dropdown_appearance' );
	...
	add_filter( 'body_class', 'biagiotti_mikado_menu_dropdown_appearance' );
}

if ( ! function_exists( 'biagiotti_mikado_header_class' ) ) {
	/**
	 ...
	 */
	function biagiotti_mikado_header_class( $classes ) {
		...
		$disable_menu_area_shadow = biagiotti_mikado_get_meta_field_intersect( 'menu_area_shadow', $id ) == 'no';
		if ( $disable_menu_area_shadow ) {
			$classes[] = 'mkdf-menu-area-shadow-disable';
		}
		
		$disable_menu_area_grid_shadow = biagiotti_mikado_get_meta_field_intersect( 'menu_area_in_grid_shadow', $id ) == 'no';
		if ( $disable_menu_area_grid_shadow ) {
			$classes[] = 'mkdf-menu-area-in-grid-shadow-disable';
		}
		
		$disable_menu_area_border = biagiotti_mikado_get_meta_field_intersect( 'menu_area_border', $id ) == 'no';
		if ( $disable_menu_area_border ) {
			$classes[] = 'mkdf-menu-area-border-disable';
		}
		
		$disable_menu_area_grid_border = biagiotti_mikado_get_meta_field_intersect( 'menu_area_in_grid_border', $id ) == 'no';
		if ( $disable_menu_area_grid_border ) {
			$classes[] = 'mkdf-menu-area-in-grid-border-disable';
		}
		
		if ( biagiotti_mikado_get_meta_field_intersect( 'menu_area_in_grid', $id ) == 'yes' &&
		     biagiotti_mikado_get_meta_field_intersect( 'menu_area_grid_background_color', $id ) !== '' &&
		     biagiotti_mikado_get_meta_field_intersect( 'menu_area_grid_background_transparency', $id ) !== '0'
		) {
			$classes[] = 'mkdf-header-menu-area-in-grid-padding';
		}
		
		$disable_logo_area_border = biagiotti_mikado_get_meta_field_intersect( 'logo_area_border', $id ) == 'no';
		if ( $disable_logo_area_border ) {
			$classes[] = 'mkdf-logo-area-border-disable';
		}
		
		$disable_logo_area_grid_border = biagiotti_mikado_get_meta_field_intersect( 'logo_area_in_grid_border', $id ) == 'no';
		if ( $disable_logo_area_grid_border ) {
			$classes[] = 'mkdf-logo-area-in-grid-border-disable';
		}
		
		if ( biagiotti_mikado_get_meta_field_intersect( 'logo_area_in_grid', $id ) == 'yes' &&
		     biagiotti_mikado_get_meta_field_intersect( 'logo_area_grid_background_color', $id ) !== '' &&
		     biagiotti_mikado_get_meta_field_intersect( 'logo_area_grid_background_transparency', $id ) !== '0'
		) {
			$classes[] = 'mkdf-header-logo-area-in-grid-padding';
		}
		
		$disable_shadow_vertical = biagiotti_mikado_get_meta_field_intersect( 'vertical_header_shadow', $id ) == 'no';
		if ( $disable_shadow_vertical ) {
			$classes[] = 'mkdf-header-vertical-shadow-disable';
		}
		
		$disable_border_vertical = biagiotti_mikado_get_meta_field_intersect( 'vertical_header_border', $id ) == 'no';
	...
	add_filter( 'body_class', 'biagiotti_mikado_header_class' );
}

if ( ! function_exists( 'biagiotti_mikado_header_area_style' ) ) {
	/**
	 * Function that return styles for header area
	 */
	function biagiotti_mikado_header_area_style( $style ) {
		...
		$menu_area_background_color_rgba = biagiotti_mikado_rgba_color( $menu_area_background_color, $menu_area_background_transparency );
		...
		if ( $menu_area_height !== '' ) {
			$menu_area_style['height'] = biagiotti_mikado_filter_px( $menu_area_height ) . 'px !important';
		}
		...
		if ( $container_side_padding !== '' ) {
			...
			$current_style .= biagiotti_mikado_dynamic_css( $menu_container_selector, $menu_container_styles );
		}
		...
		$menu_area_grid_background_color_rgba = biagiotti_mikado_rgba_color( $menu_area_grid_background_color, $menu_area_grid_background_transparency );
		
		if ( ! empty( $menu_area_grid_background_color_rgba ) ) {
			$menu_area_grid_style['background-color'] = $menu_area_grid_background_color_rgba;
		}
		
		$current_style .= biagiotti_mikado_dynamic_css( $menu_area_selector, $menu_area_style );
		$current_style .= biagiotti_mikado_dynamic_css( $menu_area_grid_selector, $menu_area_grid_style );
		...
		if ( $dropdown_top_position !== '' ) {
			$dropdown_styles['top'] = biagiotti_mikado_filter_suffix( $dropdown_top_position, '%' ) . '%';
		}
		
		$dropdown_selector = array( $class_prefix . ' .mkdf-page-header .mkdf-drop-down .second' );
		
		$current_style .= biagiotti_mikado_dynamic_css( $dropdown_selector, $dropdown_styles );
		...
		$logo_area_background_color_rgba = biagiotti_mikado_rgba_color( $logo_area_background_color, $logo_area_background_transparency );
		...
		if ( $logo_area_height !== '' ) {
			$logo_area_style['height'] = biagiotti_mikado_filter_px( $logo_area_height ) . 'px !important';
		}
		...
		$logo_area_grid_background_color_rgba = biagiotti_mikado_rgba_color( $logo_area_grid_background_color, $logo_area_grid_background_transparency );
		...
		if ( ! empty( $logo_area_style ) ) {
			$current_style .= biagiotti_mikado_dynamic_css( $logo_area_selector, $logo_area_style );
		}
		
		if ( ! empty( $logo_area_grid_style ) ) {
			$current_style .= biagiotti_mikado_dynamic_css( $logo_area_grid_selector, $logo_area_grid_style );
		}
		
		$current_style = apply_filters( 'biagiotti_mikado_filter_add_header_page_custom_style', $current_style, $class_prefix, $page_id ) . $style;
		
		return $current_style;
	}
	
	add_filter( 'biagiotti_mikado_filter_add_page_custom_style', 'biagiotti_mikado_header_area_style' );
}